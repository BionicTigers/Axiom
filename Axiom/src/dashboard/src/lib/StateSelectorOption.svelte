...
{#if value.value == undefined || typeof value.value == "number"}
    <div class="flex w-full h-fit ring-2 ring-neutral-900 rounded-lg p-2">
        ...
        {:else}
            ...
            <input type="checkbox" class="w-8 h-8 bg-neutral-800/40 ring-2 ring-neutral-900 rounded-xl outline-none p-2 backdrop-blur-lg hover:bg-neutral-800/50 transition-colors" onchange={() => {
